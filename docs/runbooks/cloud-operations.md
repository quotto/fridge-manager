...
3. `Configure Cloudflare API Edge` workflowを起動し、AOPのcertificate ID一致を確認した後でのみ、API GatewayのRegional domainをCNAMEのプロキシ有効レコードとして設定する。Cloudflare Free の Rate Limiting Ruleは両hostnameを一つのruleで扱い、`ip.src` と `cf.colo.id` ごとに `POST /v1/analysis` を合算で10回/分に制限する。
