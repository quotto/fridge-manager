...
required=(FIREBASE_PROJECT_ID FIREBASE_PROJECT_NUMBER FIREBASE_APP_IDS GOOGLE_WIF_AUDIENCE GOOGLE_SERVICE_ACCOUNT_EMAIL BUDGET_NOTIFICATION_EMAIL ACM_CERTIFICATE_ARN)
...
npx cdk deploy "$api_stack" --app "$assembly" \
  ...
  --parameters "${api_stack}:BudgetNotificationEmail=${BUDGET_NOTIFICATION_EMAIL}" \
